...
impl HalState {
    ...
    pub fn draw_clear_frame(&mut self, color: [f32; 4]) -> Result<(), String> {
        ...
        let (i_u32, i_usize) = unsafe {
            let (image_index, _) = self
                ...
                .map_err(|_| "Couldn't acquire an image from the swapchain!".to_owned())?;
        ...
        unsafe {
            self.device
                ...
                .map_err(|_| "Failed to wait on the fence!".to_owned())?;
            self.device
                .reset_fence(flight_fence)
                .map_err(|_| "Couldn't reset the fence!".to_owned())?;
        }
        ...
        unsafe {
            ...
            self.swapchain
                ...
                .map(|_| ())
                .map_err(|err| format!("Failed to present into the swapchain: {:?}", err))
        }
    }
}
...
fn do_the_render(hal_state: &mut HalState, local_state: &LocalState) -> Result<(), String> {
